<?php
	...
	class Inputs {
		...
		public static function text($name, $label, $model, $errors) {
			$value = old($name, $model[ $name ]);
			echo self::getInput($name, 'text', $label, $value, $errors);
		}
		
		public static function email($name, $label, $model, $errors) {
			$value = old($name, $model[ $name ]);
			echo self::getInput($name, 'email', $label, $value, $errors);
		}
		
		public static function password($name, $label, $model, $errors) {
			$value = old($name, $model[ $name ]);
			echo self::getInput($name, 'password', $label, $value, $errors);
		}
		
		protected static function getInput($name, $type, $label, $value, $errors) {
			...
			$rs          .= "<input type='$type' class='form-control gm-css__input gm-js__input $inputStatus' name='$name' id='gm-input__$name' value='$value'/>";
		...
		public static function number($name, $label, $min, $max, $step, $model, $errors) {
			$value = old($name, $model[ $name ]);
